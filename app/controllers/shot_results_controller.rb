class ShotResultsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @shot_result.save
        format.html { redirect_to @shot_result, notice: 'Shot result was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @shot_result.update_attributes(params[:shot_result])
        format.html { redirect_to @shot_result, notice: 'Shot result was successfully updated.' }
        format.json { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to shot_results_url }
      format.json { head :ok }
    end
  end
end
